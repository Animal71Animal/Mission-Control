...
const FILE_PATH = 'public/data/joules-todo.json';
...
async function fetchFile() {
  ...
  return { tasks: decoded.tasks, sha: data.sha };
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await writeFile(tasks, sha, `fix: update joules-todo task ${id}`);
...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { tasks, sha } = await fetchFile();
    const newTask = { id: `jc-${Date.now()}`, completed: false, ...body };
    await writeFile([...tasks, newTask], sha, `feat: add joules-todo task "${newTask.text}"`);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await writeFile(updated, sha, `chore: delete joules-todo task ${id}`);
