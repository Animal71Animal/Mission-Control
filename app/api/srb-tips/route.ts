...
function fetchFromLocal() {
  try {
    ...
    console.error('[srb-tips] Local read failed:', e);
  }
  return null;
}
...
export async function GET() {
  try {
    ...
    console.error('[srb-tips] GitHub failed, falling back to local:', err);
    const localData = fetchFromLocal();
    if (localData) {
      return NextResponse.json({ ...localData, lastUpdated: localData.lastUpdated || new Date().toISOString() });
    }
    return NextResponse.json({ monthlyTotals: [], topTippers: [], allDancers: [], nightlyData: {} }, { status: 500 });
  }
}
