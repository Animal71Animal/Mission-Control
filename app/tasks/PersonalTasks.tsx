...
async function apiPatch(id: string, changes: Partial<PersonalTask>) {
  await fetch("/api/personal-tasks", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, ...changes }) });
}
async function apiDelete(id: string) {
  await fetch(`/api/personal-tasks?id=${id}`, { method: "DELETE" });
}
async function apiAdd(task: Omit<PersonalTask, "id" | "created_at">) {
  const res = await fetch("/api/personal-tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(task) });
  return res.json();
}
...
export default function PersonalTasks() {
  ...
  useEffect(() => {
    fetch("/api/personal-tasks").then(r => r.json()).then(data => { setTasks(Array.isArray(data) ? data : []); setLoaded(true); }).catch(() => setLoaded(true));
  }, []);

  const resetForm = () => { setTitle(""); setNotes(""); setCategory("Personal"); setPriority("medium"); setDueDate(""); };
  const startEdit = (task: PersonalTask) => { setEditingId(task.id); setTitle(task.title); setNotes(task.notes || ""); setCategory(task.category); setPriority(task.priority); setDueDate(task.due_date || ""); setShowForm(true); };
  ...
  const inputStyle: React.CSSProperties = { width: "100%", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 6, padding: "10px 12px", color: "var(--text)", fontSize: "0.9rem", boxSizing: "border-box" };
  const selectStyle: React.CSSProperties = { background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 6, padding: "8px 12px", color: "var(--text)", fontSize: "0.85rem" };
    ...
    <div key={task.id} style={{ background: "var(--card)", border: "1px solid var(--border)", borderLeft: `4px solid ${CATEGORY_COLORS[task.category] || "#888"}`, borderRadius: 8, padding: "14px 16px", opacity: task.completed ? 0.6 : 1 }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          <button onClick={() => startEdit(task)} style={{ background: "rgba(0,187,249,0.1)", border: "1px solid rgba(0,187,249,0.3)", borderRadius: 4, padding: "5px 10px", fontSize: "0.75rem", color: "#00bbf9", cursor: "pointer" }}>✏️ Edit</button>
          <button onClick={() => deleteTask(task.id)} style={{ background: "rgba(224,92,92,0.1)", border: "1px solid rgba(224,92,92,0.3)", borderRadius: 4, padding: "5px 10px", fontSize: "0.75rem", color: "#e05c5c", cursor: "pointer" }}>🗑</button>
        </div>
      </div>
    </div>
  );
    ...
    <div>
      ...
      {showForm && (
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: "16px 20px", marginBottom: 20 }}>
          ...
          <input type="text" placeholder="Task title..." value={title} onChange={e => setTitle(e.target.value)} style={{ ...inputStyle, marginBottom: 10 }} onKeyDown={e => e.key === "Enter" && handleSubmit()} />
          <textarea placeholder="Notes (optional)..." value={notes} onChange={e => setNotes(e.target.value)} rows={2} style={{ ...inputStyle, marginBottom: 10, resize: "vertical" }} />
          <div style={{ display: "flex", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
            <select value={category} onChange={e => setCategory(e.target.value)} style={selectStyle}>{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={handleSubmit} disabled={saving || !title.trim()} style={{ background: saving ? "#555" : "#00c87c", color: "#fff", border: "none", borderRadius: 6, padding: "9px 18px", fontSize: "0.85rem", fontWeight: 600, cursor: saving ? "default" : "pointer" }}>
              {saving ? "Saving..." : editingId ? "Save Changes" : "Add Task"}
            </button>
            <button onClick={cancelForm} style={{ background: "transparent", color: "var(--muted)", border: "1px solid var(--border)", borderRadius: 6, padding: "9px 18px", fontSize: "0.85rem", cursor: "pointer" }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 6, padding: "6px 10px", color: "var(--text)", fontSize: "0.8rem" }}>
        ...
        <div>
          ...
          {animalOpen && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 8 }}>
              {myTasks.length === 0 ? <div style={{ color: "var(--muted)", padding: "20px", textAlign: "center", fontSize: "0.85rem" }}>No tasks here.</div> : myTasks.map(renderTask)}
            </div>
          )}
        </div>
        <div>
          ...
          {priscyllaOpen && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 8 }}>
              {priscyllaTasks.length === 0 ? <div style={{ color: "var(--muted)", padding: "20px", textAlign: "center", fontSize: "0.85rem" }}>No tasks here.</div> : priscyllaTasks.map(renderTask)}
            </div>
          )}
        </div>
      </div>
